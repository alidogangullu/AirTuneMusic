...
class AudioRenderer {
    ...
    private fun ensureSoftwareAlac(ct: Int): Boolean {
    ...
    private fun start(ct: Int) {
        ...
        val format = when (ct) {
            CT_AAC_ELD -> MediaFormat.createAudioFormat(MediaFormat.MIMETYPE_AUDIO_AAC, 44100, 2).apply {
            ...
            CT_AAC_LC  -> MediaFormat.createAudioFormat(MediaFormat.MIMETYPE_AUDIO_AAC, 44100, 2).apply {
                setInteger(MediaFormat.KEY_AAC_PROFILE, 2)
                setInteger(MediaFormat.KEY_IS_ADTS, 0)
                setByteBuffer("csd-0", ByteBuffer.wrap(byteArrayOf(0x12, 0x10)))
            }
            CT_ALAC    -> MediaFormat.createAudioFormat("audio/alac", 44100, 2).apply {
            ...
            else -> { Log.w(TAG, "Unknown audio ct=$ct"); return }
        }
        ...
        try {
            ...
            codec    = null
            failedCt = ct
            if (ct == CT_ALAC && swAlacEnabled) { startSoftwareAlac(); return }
            return
        }
        ensureAudioTrack()
        Log.i(TAG, "Audio started: $codecLabel")
    }
