...
dnssd_init(const char *name, int name_len, const char *hw_addr, int hw_addr_len,
           int *error, int require_pw)
{
    if (error) *error = DNSSD_ERROR_NOERROR;

    dnssd_t *dnssd = (dnssd_t *)calloc(1, sizeof(dnssd_t));
    if (!dnssd) { if (error) *error = DNSSD_ERROR_OUTOFMEM; return NULL; }

    dnssd->pin_pw = (unsigned char)require_pw;
    strncpy(dnssd->codec_cn, RAOP_CN, sizeof(dnssd->codec_cn) - 1);

    char *end = NULL;
    unsigned long f = strtoul(FEATURES_1, &end, 16);
    if (!end) { free(dnssd); if (error) *error = DNSSD_ERROR_BADFEATURES; return NULL; }
    dnssd->features1 = (uint32_t)f;

    f = strtoul(FEATURES_2, &end, 16);
    if (!end) { free(dnssd); if (error) *error = DNSSD_ERROR_BADFEATURES; return NULL; }
    dnssd->features2 = (uint32_t)f;

    dnssd->name_len = name_len;
    dnssd->name = (char *)calloc(1, name_len + 1);
    if (!dnssd->name) { free(dnssd); if (error) *error = DNSSD_ERROR_OUTOFMEM; return NULL; }
    memcpy(dnssd->name, name, name_len);

    dnssd->hw_addr_len = hw_addr_len;
    dnssd->hw_addr = (char *)calloc(1, hw_addr_len);
    if (!dnssd->hw_addr) {
        free(dnssd->name); free(dnssd);
...
int dnssd_register_raop(dnssd_t *dnssd, unsigned short port) {
    ...
    switch (dnssd->pin_pw) {
    case 2: case 3:
        _txt_set(rec, "pw", "true"); _txt_set(rec, "sf", "0x84"); break;
    case 1:
        _txt_set(rec, "pw", "true"); _txt_set(rec, "sf", "0x8c"); break;
    default:
        _txt_set(rec, "pw", "false"); _txt_set(rec, "sf", RAOP_SF); break;
    }
    ...
    if (dnssd->pk) _txt_set(rec, "pk", dnssd->pk);

    /* Build service name: HWADDR@Name */
    if (utils_hwaddr_raop(dnssd->raop_servname, sizeof(dnssd->raop_servname),
                          dnssd->hw_addr, dnssd->hw_addr_len) < 0) {
        return -1;
    }
    strncat(dnssd->raop_servname, "@",
            sizeof(dnssd->raop_servname) - strlen(dnssd->raop_servname) - 1);
    strncat(dnssd->raop_servname, dnssd->name,
            sizeof(dnssd->raop_servname) - strlen(dnssd->raop_servname) - 1);
...
int dnssd_register_airplay(dnssd_t *dnssd, unsigned short port) {
    ...
    case 1: case 2: case 3:
        _txt_set(rec, "pw", "true"); break;
    default:
        _txt_set(rec, "pw", "false"); break;
    }
...
void dnssd_set_airplay_features(dnssd_t *dnssd, int bit, int val) {
    ...
    uint32_t mask;
    uint32_t *features;
    if (bit >= 32) { mask = 1u << (bit - 32); features = &dnssd->features2; }
    else           { mask = 1u << bit;         features = &dnssd->features1; }
    if (val) *features |= mask; else *features &= ~mask;
}
